    ...
    func winner(stateHistory: [State]) -> Player? {
        ...
        for i in 0 ..< size {
            for j in 0 ..< size {
                ...
                // Go Right
                if checkWiner(line: { (k: Int) -> Move in Move(x: i, y: k + j) }) {
                    return currentPlayer
                }
                // Go Down
                if checkWiner(line: { (k: Int) -> Move in Move(x: i + k, y: j) }) {
                    return currentPlayer
                }
                // Go Left-Down
                if checkWiner(line: { (k: Int) -> Move in Move(x: i + k, y: j - k) }) {
                    return currentPlayer
                }
                // Go Right-Down
                if checkWiner(line: { (k: Int) -> Move in Move(x: i + k, y: j + k) }) {
