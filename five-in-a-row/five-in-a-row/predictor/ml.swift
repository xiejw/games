...
// The wrapper class to predict distribution and reward.
class DistributionPredictionForLastIterationWrapper: Predictor {
    private var size: Int

    // The CoreML model.
    private let model = DistributionLastIteration()
    ...
    func predictDistributionAndNextPlayerReward(state: State) -> ([Double], Double) {
        ...
        // FIXME. Most of the old is same. Refactoring.
        let output = try! model.prediction(input:
            DistributionLastIterationInput(board: mlMultiArrayInput!, next_player: mlMultiInput!))
