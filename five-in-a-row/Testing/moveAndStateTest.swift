...
class MoveAndStateTest: XCTestCase {
    ...
    func testMoveInequality() {
        let move1 = Move(x: 2, y: 3)
        let move2 = Move(x: 1, y: 3)
        XCTAssertFalse(move1 < move2)

        let move3 = Move(x: 3, y: 2)
        XCTAssertTrue(move1 < move3)

        let move4 = Move(x: 2, y: 2)
        XCTAssertFalse(move1 < move4)
    }
